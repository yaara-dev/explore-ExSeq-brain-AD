%% Compare gene expression between WT and 5xFAD mice across mapped brain regions
% Each Parquet file contains transcript-level rows with at least:
%   gene
%   region_name
%
% Optional:
...
%
% The script:
%   1. Reads Parquet row groups for WT and 5xFAD mice
%   2. Standardizes gene names before counting
%   3. Merges known gene aliases before counting
%   4. Removes blank/unmapped annotations before counting
%   5. Retains every mapped brain region and biological gene
%   6. Merges multiple sections that belong to the same mouse
%   8. Counts expression per gene and mapped brain region
...
%   12. Compares 5xFAD vs WT within each mapped brain region
...
% true  = compare per gene, mapped brain region, and cell type
useCellTypes = false;
...
useFoldChangeFiltering = false;
foldChangeCutoff = 1.1;

useMinimumMeanExpressionFiltering = true;
minimumMeanExpression = 100;

useSampleSupportFiltering = true;
minimumNumberOfExpressingSamples = 2;
sampleSupportExpressionThreshold = 10;
...
% The full files contain hundreds of millions of transcript rows. Gene-name
% diagnostics that reread every row are intentionally disabled; cleaning and
% counting are performed together during the single row-group pass below.
...
for i = 1:numel(samples)
    ...
    sampleTables{i} = readAndCountMouseParquet( ...
        samples(i).files, ...
        inputFolder, ...
        samples(i).name, ...
        useCellTypes);
...
writeTableScalable(rawExpressionTable, outputFolder, ...
    "raw_counts_gene_region_per_mouse");
...
fprintf("Total mapped regions: %d\n", height(geneCountByRegion));
...
writeTableScalable(analysisExpressionTable, outputFolder, ...
    "analysis_expression_gene_region_per_mouse");
...
writeTableScalable(allResultsTable, outputFolder, ...
    "all_WT_vs_5xFAD_region_results");
...
function sampleTable = readAndCountMouseParquet(fileList, inputFolder, sampleName, useCellTypes)

    if useCellTypes
        requiredCols = {'gene', 'region_name', 'reduced_cell_type'};
    else
    ...
    for f = 1:numel(fileList)
        ...
        info = parquetinfo(currentPath);
        missingCols = setdiff(string(requiredCols), info.VariableNames);
        ...
        fprintf("  %s: %d row groups\n", currentFile, info.NumRowGroups);

        for firstGroup = 1:rowGroupBatchSize:info.NumRowGroups
            rowGroups = firstGroup:min(firstGroup + rowGroupBatchSize - 1, ...
                info.NumRowGroups);

            T = parquetread(currentPath, ...
                "SelectedVariableNames", requiredCols, ...
                "RowGroups", rowGroups);

            if useCellTypes
                T.Properties.VariableNames{'reduced_cell_type'} = 'cell_type';
            end

            T = cleanInputTable(T, useCellTypes);
            chunkCounts = makeCountTable(T, useCellTypes);

            if isempty(sampleCounts)
                sampleCounts = chunkCounts;
            else
                sampleCounts = collapseCountTable( ...
                    [sampleCounts; chunkCounts], useCellTypes);
            end

            fprintf("    counted row groups %d-%d of %d\n", ...
                rowGroups(1), rowGroups(end), info.NumRowGroups);
        end
    end

    countTable = collapseCountTable(sampleCounts, useCellTypes);
...
function countTable = collapseCountTable(countTable, useCellTypes)

    if isempty(countTable)
        return;
    end

    if useCellTypes
        [G, geneNames, regionNames, cellTypes] = findgroups( ...
            countTable.gene, countTable.region_name, countTable.cell_type);
        copyCounts = splitapply(@sum, countTable.copy_count, G);
        countTable = table(geneNames, regionNames, cellTypes, copyCounts, ...
            'VariableNames', {'gene', 'region_name', 'cell_type', 'copy_count'});
    else
        [G, geneNames, regionNames] = findgroups( ...
            countTable.gene, countTable.region_name);
        copyCounts = splitapply(@sum, countTable.copy_count, G);
        countTable = table(geneNames, regionNames, copyCounts, ...
...
function T = cleanInputTable(T, useCellTypes)
    ...
    %% Exclude Xenium control codewords, but retain every biological gene
    controlGeneRows = ...
        startsWith(T.gene, "NEGCONTROL") | ...
        startsWith(T.gene, "UNASSIGNEDCODEWORD") | ...
        startsWith(T.gene, "DEPRECATEDCODEWORD") | ...
        startsWith(T.gene, "SYSTEMCONTROL") | ...
        startsWith(T.gene, "BLANK-");
    ...
    %% Exclude only annotations that are not mapped brain regions
    regionLower = lower(strtrim(T.region_name));
    excludeRegionRows = ismember(regionLower, ...
        ["unassigned", "unmapped", "unknown", "outside"]);
...
function writeTableScalable(T, outputFolder, baseName)

    % Parquet has no Excel row limit and preserves types for large outputs.
    parquetPath = fullfile(outputFolder, baseName + ".parquet");
    parquetwrite(parquetPath, T);

    maximumExcelDataRows = 1048575;
    if height(T) <= maximumExcelDataRows
        writetable(T, fullfile(outputFolder, baseName + ".xlsx"));
    else
        fprintf("Skipped Excel copy of %s (%d rows exceeds Excel limit).\n", ...
            baseName, height(T));
    end
end
