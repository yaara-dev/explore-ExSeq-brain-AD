%% Compare Morans I between WT and 5xFAD mice by hippocampal region
...
%   4. Calculates Morans I per gene, region, and animal
%   5. Aggregates RNA molecules into 10 um cubic voxels
%   6. Uses six-neighbor voxel connectivity: +/-x, +/-y, +/-z
%   7. Quantile-normalizes Morans I values across animals
%   8. Compares WT vs 5xFAD per gene-region pair
%   9. Supports:
%        - empirical-Bayes moderated t-test, limma-like
%        - Welch t-test
%        - permutation test
%   10. Applies global BH-FDR correction across all tested gene-region pairs
%
...
minimumMoleculesPerGeneRegionAnimal = 5;
...
% With 6 WT and 6 5xFAD, exhaustive permutation gives nchoosek(12,6)=924.
...
useMeanDifferenceQuantileFilter = true;
...
useAbsDiffNormalizedFilter = false;
absDiffNormalizedCutoff = 0.06;
...
% Multiple files listed for one sample are merged before Moran calculation.
% =============================
...
samples(2).group = "WT";
samples(2).files = {
    "WT_animal2_sectionA.csv"
    "WT_animal2_sectionB.csv"
    };
...
samples(6).group = "WT";
samples(6).files = {
    "WT_animal6_sectionA.csv"
    "WT_animal6_sectionB.csv"
    };
...
samples(10).name = "FAD4";
samples(10).group = "FAD";
samples(10).files = {
    "FAD_animal4.csv"
    };

samples(11).name = "FAD5";
samples(11).group = "FAD";
samples(11).files = {
    "FAD_animal5.csv"
    };

samples(12).name = "FAD6";
samples(12).group = "FAD";
samples(12).files = {
    "FAD_animal6.csv"
    };
...
rawMoranWideTable = buildWideMoranTable(allMoranLong, sampleNames);

writetable(rawMoranWideTable, ...
    fullfile(outputFolder, "Moran_I_raw_wide.xlsx"));
...
function T = readAndCleanMoranInputFiles(fileList, inputFolder)
    ...
    for f = 1:numel(fileList)
        ...
        xCol = findFirstColumn(varNames, ["global_x_um", "x_um", "x"]);
        yCol = findFirstColumn(varNames, ["global_y_um", "y_um", "y"]);
        zCol = findFirstColumn(varNames, ["Z_um", "global_z_um", "z_um", "z"]);
...
function regionMoranTable = calculateMoranForOneRegion( ...
    ...
    if numberOfVoxels > maximumNumberOfVoxelsPerRegion
        error("Region %s in sample %s requires %d voxels. Increase voxel size or maximumNumberOfVoxelsPerRegion.", ...
            regionName, sampleName, numberOfVoxels);
    end
